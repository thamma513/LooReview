...
},
{
  ...
  render(){
    const MapItems = this.state.markers.map(location => {
      return(
      
      <Card class="card" className='blue-grey darken-1' textClassName='white-text' title='Card title' title={location.name}> {location.address} </Card>);
